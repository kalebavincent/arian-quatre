...
#         # Dynamically arrange buttons in columns (e.g., 2 buttons per row)
#         columns = get_row()
#         button_rows = []
#         for idx, btn in enumerate(buttons):
#             if hasattr(btn, 'name') and hasattr(btn, 'url'):
#                 if idx % columns == 0:
#                     button_rows.append([])
#                 button_rows[-1].append(InlineKeyboardButton(btn.name, url=btn.url))

#         # Add buttons to the InlineKeyboardMarkup
#         reply_markup = InlineKeyboardMarkup(button_rows)
        
#         # Sending the promo message with dynamic buttons
...
#         )
#         await message.reply("✅ Message envoyé avec des colonnes dynamiques.")
#     except Exception as e:
#         LOGGER.error(e)
#         await bot.send_message(
#             LOG_CHANNEL,
...
#             parse_mode=enums.ParseMode.HTML,
#         )
#         await message.reply("**⚠️ Une erreur est survenue lors de l'envoi du message.**", parse_mode=enums.ParseMode.MARKDOWN)

# Variable globale pour stocker les IDs des messages envoyés
sent_message_ids = []
